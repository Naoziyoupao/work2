...
DEFAULT_CONFIG = {
    ...
    "train_file": "data/processed/oasst2/classification/train.jsonl",
    "val_file": "data/processed/oasst2/classification/val.jsonl",
    "output_dir": "./outputs/experiments/exp001_classifier",
...
class TextClassificationDataset(Dataset):
    ...
    def __init__(self, jsonl_file, tokenizer, max_length=1024, preload=True, cache_dir=None):
        ...
        if cache_dir:
            ...
            import hashlib
            cache_id = hashlib.md5(f"{jsonl_file}_{max_length}".encode()).hexdigest()
    ...
    def _encode_item(self, item):
        """将单个样本编码为模型输入"""
        # 支持两种数据格式：
        # 1. 包含 "text" 字段的格式（prepare_classification_data.py生成）
        # 2. 包含 "instruction" 和 "response" 字段的格式（match_and_merge_datasets.py生成）
        if "text" in item:
            text = item["text"]
        else:
...
class ClassifierWithLoRA(nn.Module):
    """使用LoRA的分类器模型，带有自定义分类头"""
    def __init__(self, model_id, num_labels=2, use_lora=True, tokenizer=None):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        # 确保索引不会越界
        seq_lengths = torch.clamp(seq_lengths, max=hidden_states.shape[1]-1)
        
        # 收集每个序列的最后一个token的隐藏状态
        pooled_output = torch.stack([hidden_states[i, seq_lengths[i], :] for i in range(batch_size)])
        ...
        # 返回与transformers模型输出格式兼容的结果，但不包含hidden_states以节省内存
...
def train():
    """主训练函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="训练二分类器模型")
    ...
    logging.info(f"✅ 训练完成，模型已保存至 {args.output_dir}")
